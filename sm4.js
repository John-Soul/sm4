...
// 算法核心
const SM4 = function() {};
SM4.fn = SM4.prototype = {
  constructor: sm4,
  SM4_ENCRYPT: 1,
  SM4_DECRYPT: 0,
  SboxTable: [0xd6, 0x90, 0xe9, 0xfe, 0xcc, 0xe1, 0x3d, 0xb7, 0x16, 0xb6, 0x14, 0xc2, 0x28, 0xfb, 0x2c, 0x05,
  ...
  secretKey: "ppUf31pv4bU9H5Yk", // 后端定义
  iv: "",
  hexString: false,
  mode: 1,
  isPadding: true,
  ...
  sm4F: function (x0, x1, x2, x3, rk) {
    return x0 ^ this.sm4Lt(x1 ^ x2 ^ x3 ^ rk);
  },

  sm4CalciRK: function (ka) {
    let bb = 0;
    let rk = 0;
    const a = new Array(4);
    const b = new Array(4);
  ...
  sm4Sbox: function (inch) {
    var i = inch & 0xFF;
    var retVal = this.SboxTable[i];
  ...
  sm4_crypt_ecb: function (ctx, input) {
    if (input == null) {
      alert("input is null!");
    }
  ...
  // 加密_ECB
  encrypt_ECB: function (plainText) {
    try {
      this.isPadding = true;
      this.mode = this.SM4_ENCRYPT;
      const keyBytes = this.stringToByte(this.secretKey);
      this.sm4_setkey_enc(this, keyBytes);
      const encrypted = this.sm4_crypt_ecb(this, this.stringToByte(plainText));
  ...
  // 解密_ECB
  decrypt_ECB: function (cipherText) {
    try {
      this.isPadding = true;
      this.mode = this.SM4_ENCRYPT;
      const keyBytes = this.stringToByte(this.secretKey);
      this.sm4_setkey_dec(this, keyBytes);
      const decrypted = this.sm4_crypt_ecb(this, base64js.toByteArray(cipherText));
  ...
  encrypt_CBC: function (plainText) {
    try {
      this.isPadding = true;
      this.mode = this.SM4_ENCRYPT;
      ...
      this.sm4_setkey_enc(ctx, keyBytes);
      var encrypted = this.sm4_crypt_cbc(ctx, ivBytes, this.stringToByte(plainText));
  ...
  // 解密_CBC
  decrypt_CBC: function (cipherText) {
    try {
      this.isPadding = true;
      this.mode = this.SM4_ENCRYPT;
      const keyBytes = this.stringToByte(this.secretKey);
      ...
      this.sm4_setkey_dec(this, keyBytes);
      var decrypted = this.sm4_crypt_cbc(this, ivBytes, base64js.toByteArray(cipherText));
      return this.byteToString(decrypted);
    } catch (e) {
      console.error(e);
      return null;
...
const init = SM4.fn.init = function () {
  this.mode = 1;
  this.isPadding = true;
...
init.prototype = SM4.fn;

export const sm4 = new SM4.fn.init();